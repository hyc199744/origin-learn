...
(function(){
  ...
  var css="#live-wrap *{box-sizing:border-box}"
    ...
    +"#live-wrap .lv-rep-tabs{display:inline-flex;background:rgba(0,0,0,.28);border:1px solid var(--line);border-radius:12px;padding:4px;gap:3px;margin-bottom:16px;max-width:100%}"
    +"#live-wrap .lv-tab{padding:7px 16px;border-radius:9px;font-size:13.5px;font-weight:700;text-decoration:none;cursor:pointer;white-space:nowrap;transition:.18s;color:var(--soft)}"
    +"#live-wrap .lv-tab.on{background:linear-gradient(135deg,var(--glt),#b8842f);color:#1a1206}"
    +"#live-wrap .lv-tab.off:hover,#live-wrap .lv-tab.off:focus{color:var(--glt);outline:none}"
    +"@media(max-width:400px){#live-wrap .lv-tab{padding:7px 12px;font-size:12.5px}}"
  ...
  function renderShell(){
    ...
    root.innerHTML='<div id="live-wrap">'
      ...
      +'<div class="lv-section" id="lv-replays-sec" style="display:none"><div class="lv-rep-tabs"><a class="lv-tab on" aria-current="page">🎧 '+esc(T.audio_replays_title)+'</a><a class="lv-tab off" href="/tools/video-replays/">🎬 '+esc(T.video_replays_title)+'</a></div><div class="lv-rep-grid" id="lv-replays"></div></div>'
  ...
  // 免费直播页只显示「音频回放」;视频回放为会员专属,在 /tools/video-replays/ 里看
  function renderReplays(list){
    var sec=document.getElementById("lv-replays-sec"),grid=document.getElementById("lv-replays");
    if(!sec||!grid)return;
    state.replays=list||[];
    var auds=(list||[]).filter(function(c){return c&&c.play_url&&!(c.kind==="video"||c.kind==="embed");});
    sec.style.display="";
    fillGrid(auds,grid,T.no_replays);
  }
