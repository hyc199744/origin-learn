...
function liveForm(){
  ...
  wrap.innerHTML='<div class="a-panel-box" id="lvForm"><h3>'+(_liveEdit?"编辑课程 #"+c.id:"新建课程")+'</h3>'
    ...
    +'<label class="lv-lb">嵌入模式</label><select class="lv-in" id="fMode">'
      +'<option value="auto"'+(c.embed_mode!=="embed"?" selected":"")+'>页面内嵌入（全程留在本站，不跳转原网站）</option>'
      +'<option value="embed"'+(c.embed_mode==="embed"?" selected":"")+'>页面内嵌入（同上）</option></select>'
...
function pDash(el){
  ...
  get("/overview?range="+encodeURIComponent(AN_RANGE)).then(function(r){
    ...
    w3oLiveOverview(el.querySelector("#ovBody"));
  }).catch(function(){});
}
function w3oLiveOverview(host){
  if(!host)return;
  get("/livestats").then(function(ls){if(!host||!ls||!ls.ok)return;var st=ls.stats||{},mz=st.mengzhu||{},cl=st.cloud||{};
